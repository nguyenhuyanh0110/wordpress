...
function shopping_mart_general_admin_notice(){
	...
	if($pagenow == 'themes.php' || $pagenow == 'index.php'  || $pagenow == 'plugins.php' || $pagenow == 'options-general.php'){
			 ...
			 $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible" >
					...
					esc_html__('See Premium Features','shopping-mart'),					
					esc_html__('Always Hide this notice', 'shopping-mart'),					
					esc_html__('1. Use page builder such as elementor, beaver builder and drag and drop Theme:Product Categories widget to build WooCommerce content. 2. In Customize -> Theme Options customize Slider, Navigation and Banner section.', 'shopping-mart')				
			 );
...
//show, hide notice, delete_option('mart_notice_dismissed');
if ( isset( $_GET['mart_notice_dismissed'] ) ){
	update_option('mart_notice_dismissed', 11);
}

$shopping_mart_notice = absint(get_option('mart_notice_dismissed', 0));

if($shopping_mart_notice != 11){
